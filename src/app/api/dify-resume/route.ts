  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // /form/human_input/ は 200 返すが実際に再開しないため、新ランで対処
          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (data.event === 'workflow_started') {
                ...
                // Human Input ノードを検知した場合（selected_urls がバイパスされなかった）
                if (title === 'URL選択' || title.includes('Human') || title.includes('human')) {
                  log(`⚠️ Human Inputノード検知（新規ラン）: "${title}" → selected_urls がバイパスされなかった可能性`);
                  send({ progress: progressVal, status: `⚠️ Human Inputノード再検知: "${title}"（URLが渡されていない可能性）` });
                } else {

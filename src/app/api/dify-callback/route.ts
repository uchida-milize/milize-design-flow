...
export async function POST(req: NextRequest) {
  ...
  // _reset: true が含まれる場合は既存リソースをクリアして新規データのみ書き込む
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { client_slug: _cs, _reset, ...incoming } = body;
  const shouldReset = _reset === true;

  const filePath = `src/app/${client_slug}/resources.json`;

  // ── 既存の resources.json を取得してマージ（_reset 時はスキップ）──────────────
  ...
  // 新しいデータをマージ（_reset 時は既存を破棄して incoming のみ）

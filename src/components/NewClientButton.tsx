...
export function NewClientButton() {
  ...
  /** フェーズ1: /api/dify-urls でURL一覧を収集してURL選択ステップへ */
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setStep('generating');
    setGenProgress(5);
    setGenStatus('URLを収集中...');
    ...
    const buildUrlItems = (difyUrls: string[]) => {
      ...
      const filtered = difyUrls.filter(u => !presets.includes(u) && u !== wikiUrl);
      const { items, excluded } = initUrlItems(filtered);
      return { items: [wikiItem, ...presetItems, ...items], excluded };
    };
    ...
    try {
      ...
      await consumeSseStream(res, (data) => {
        // dify-urls sends { progress: 100, status: '収集完了', urls: UrlItem[] }
        const rawUrls = data.urls;
        if (Array.isArray(rawUrls) && rawUrls.length > 0) {
          const urlStrings = (rawUrls as Array<{ url?: string } | string>)
            .map(u => (typeof u === 'string' ? u : (u as { url?: string }).url ?? ''))
            .filter(Boolean);
          const { items, excluded } = buildUrlItems(urlStrings);
          setUrlItems(items);
    ...
    if (!urlsReceived) {
      // Fallback: show URL selection with only wiki + user-presets
      const { items, excluded } = buildUrlItems([]);
      setUrlItems(items);
      setExcludedCount(excluded);
      setStep('urls');
      setGenStatus('');
  ...
  /** フェーズ2: 選択URLを /api/generate-portal に送ってポータルを生成 */
  async function handleResume() {
    setUrlSelectionDone(true);
    setStep('generating');
    setGenStatus('デザイントークンを収集中...');

    // 選択済みURLを "URL||カテゴリ1,カテゴリ2" 形式にして送信
    const selectedUrlsStr = urlItems
      .filter(u => u.checked)
      .map(u => {
    ...
    // フォールバック: deploy_done が来なかった場合もモーダルを閉じてリフレッシュ
    setGenProgress(100);
  ...
  async function consumeSseStream(
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (data.error) setGenStatus('エラー: ' + String(data.error));
          if (onData(data)) { clearTimeout(timeoutId); return true; }
        } catch { /* skip */ }
      }
    }
    ...
    // dify_done: Vercel のポーリングを自前で実施（Vercelトークン未設定時のフォールバック）
    if (difyDone) {
      const slug = form.client_slug;
      let deployed = false;
      let attempts = 0;
    ...
    <>
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '13px', color: '#9ca3af', marginTop: '4px' }}>URLを選択してポータルを自動生成</div>
        </div>
        <a
            ...
            {step === 'generating' && (
              <>
                ...
                <p style={{ fontSize: '14px', color: '#777', marginBottom: '32px' }}>
                  {form.company_name}{'のポータルを構築しています'}
                </p>
